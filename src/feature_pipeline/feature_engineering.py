...
def target_encode(
    ...
    """
    Target encode a categorical column using the mean of the target.
    Always outputs `city_full_encoded` to keep schema consistent.
    Fit on train only, apply to eval.
    """
    te = TargetEncoder(cols=[col])

    train["city_full_encoded"] = te.fit_transform(train[col], train[target])
    eval["city_full_encoded"] = te.transform(eval[col])
