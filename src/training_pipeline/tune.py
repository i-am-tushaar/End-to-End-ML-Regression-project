...
- Logs Optuna trials to MLflow
- Retrains best model and saves it
"""
...
def _normalize_tracking_uri(tracking_uri: Optional[str]) -> None:
    if not tracking_uri:
        return
    ...
    mlflow.set_tracking_uri(f"file:///{path.as_posix()}")
...
def tune_model(
    ...
    # --- MLflow setup ---
    _normalize_tracking_uri(tracking_uri)
    ...
    # --- Log final model ---
    with mlflow.start_run(run_name="best_xgb_model", nested=False):
